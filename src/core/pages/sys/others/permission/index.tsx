...
export default function PermissionPage() {
	const { nombre } = useUserInfo();
	// const signIn = useSignIn();
	const { check, checkAny, checkAll } = useAuthCheck();

	const handleSwitch = (_username: string) => {
		if (_username === nombre) return;
	...
	return (
		<div className="flex flex-col gap-4">
			<div className="w-full flex  items-center justify-center">
				...
				<Tabs defaultValue={nombre} onValueChange={handleSwitch}>
					<TabsList>
						{DB_USER.map((user) => (
			...
			<Card>
				<CardContent>
					<div className="flex items-center gap-2">
						...
						<Text variant="body1">[基于rolId的简化系统]</Text>
					</div>
					<div className="flex items-center gap-2">
						<Text variant="body1">当前用户权限：</Text>
						<Text variant="body1">[使用usePermissions hook]</Text>
					</div>
				</CardContent>
			</Card>
